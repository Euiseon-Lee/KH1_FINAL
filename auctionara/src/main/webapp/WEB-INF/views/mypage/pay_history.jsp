...
<!-- 사이드바 -->
	...
	<div class="row flex-fill d-flex flex-column">
		<div class="col">
        	<h4 class="row fw-bold my-4 pt-2">내 입찰 내역</h4>
        	<div class="row mb-4">
        		<div class="col-4 fw-bold">
        			총 참여 경매 <span class="text-info fs-large ml-2">{{ comma(totalAuction) }}건</span>
        		</div>
	        	...
	        	<table class="table table-hover border-bottom">
					...
					<tbody>
				    	<tr v-for="(auction, index) in list" :key="index">
				    		<td class="text-muted fs-small">{{ dateFormat(auction.auctionClosedTime) }}</td>
				    		...
				    		<td class="fs-small fw-bold">
				    			<span class="text-muted" v-if="auction.auctionProcess == 0 && auction.succBidStatus == 0 && auction.succBidNo == 0">입찰 중</span>
				    			<span class="text-muted" v-if="auction.auctionProcess == 0 && auction.succBidStatus == 0 
				    				&& auction.succBidNo != 0 && auction.topBidder == '0'">낙찰 실패</span>
				    			<span class="text-info" v-if="auction.auctionProcess == 0 && auction.succBidStatus == 0 
				    				&& auction.succBidNo != 0 && auction.topBidder == '1'">결제 전</span>
				    			<a :href="'${root}/chat/' + auction.auctionNo"><span class="text-primary" v-if="auction.auctionProcess == 0 && auction.succBidStatus == 1  && auction.topBidder == '1' && 
				    				(auction.succAuctioneerApprove == null || auction.succBidderApprove  == null)">거래 중</span></a>
				    			<a :href="'${root}/chat/' + auction.auctionNo"><span class="text-success" v-if="auction.auctionProcess == 0 && auction.succBidStatus == 1 && auction.topBidder == '1'
				    				&& auction.succAuctioneerApprove != null && auction.succBidderApprove != null && auction.ratingNo == 0">평가 전</span></a>
				    			<span class="text-success" v-if="auction.auctionProcess == 0 && auction.succBidStatus == 1 
				    				&& auction.topBidder == '1' && auction.ratingNo != 0">평가 완료</span>
				    			<span class="text-muted" v-if="auction.auctionProcess == 0 && auction.succBidStatus == 2 && auction.topBidder == '1'">미결제</span>
				    			<span class="text-muted" v-if="auction.auctionProcess == 2">경매 중지</span>
				    		</td>
				    		<td class="fs-small fw-bold text-muted">
				    			<span class="d-flex align-self-center">{{ comma(auction.maxBiddingPrice) }}원
				    				<i v-if="auction.topBidder == '1'" class="fa-solid fa-crown ml-2 py-1 text-warning"></i>
				    			</span>
				    		</td>
				    	</tr>			    			    
					</tbody>
	...
	const app = Vue.createApp({
        ...
        methods: {
            ...
            loadFirst() { // 첫 리스트 불러오기
            	axios.get("http://localhost:8080/auctionara/mypage/pay_history/list", {
            		params: {
            			page: this.page,
            			filter: this.filter,
            			sort: this.sort,
